...
int main(int argc, char** argv)
{
  ...
  if (poptions->pocc_interpreter)
      ...
      if (poptions->trash)
	{
	  ...
	  poptions->ponos == 0 && poptions->psimdkzer == 0)
	if (! poptions->quiet)
      ...
      if (poptions->cdscgr)
	{
      ...
      else if (poptions->ddg_analyze)
	{
	  pocc_driver_ddganalyze (poptions->input_file, poptions, puoptions);
	}
      else
	{
#endif
	  ...
	  // (2) Preprocess the SCoP.
	  if (poptions->scop_preprocess)
	    pocc_driver_preprocess (scop, poptions, puoptions);
